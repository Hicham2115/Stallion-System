...
router.post('/login', async (req, res): Promise<void> => {
  ...
  const portalUser = await prisma.clientPortalUser.findUnique({
    ...
    include: { client: { select: { id: true, name: true, service: true, monthlyFee: true, status: true, startDate: true, contactPerson: true, email: true, preferredCurrency: true } } },
  });

  if (!portalUser || !portalUser.active) { res.status(401).json({ message: 'Invalid credentials' }); return; }
  ...
  await prisma.clientPortalUser.update({ where: { id: portalUser.id }, data: { lastLogin: new Date() } });

  const token = jwt.sign(
    { clientPortalUserId: portalUser.id, clientId: portalUser.clientId, type: 'portal' },
...
router.get('/me', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const portalUser = await prisma.clientPortalUser.findUnique({
    where: { id: req.portalUser!.clientPortalUserId },
    include: { client: { select: { id: true, name: true, service: true, monthlyFee: true, status: true, startDate: true, contactPerson: true, email: true, preferredCurrency: true } } },
  });
  if (!portalUser) { res.status(404).json({ message: 'Not found' }); return; }
  const { password: _, ...safeUser } = portalUser;
  res.json(safeUser);
});

router.put('/profile', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.put('/change-password', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { oldPassword, newPassword } = req.body;
  if (!oldPassword || !newPassword) { res.status(400).json({ message: 'Both passwords required' }); return; }
  const user = await prisma.clientPortalUser.findUnique({ where: { id: req.portalUser!.clientPortalUserId } });
  ...
  await prisma.clientPortalUser.update({ where: { id: user.id }, data: { password: hashed } });
...
router.get('/dashboard', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { clientId, clientPortalUserId } = req.portalUser!;

  const [client, payments, recentUpdates, pendingApprovals, unreadCount] = await Promise.all([
    ...
    prisma.contentDelivery.count({ where: { clientId, status: 'WAITING_APPROVAL' } }),
    prisma.clientNotification.count({ where: { clientPortalUserId, read: false } }),
  ]);
...
router.get('/updates', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.post('/updates/:id/comments', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { content } = req.body;
  if (!content?.trim()) { res.status(400).json({ message: 'Comment content required' }); return; }

  const update = await prisma.projectUpdate.findUnique({ where: { id: req.params.id } });
  if (!update || update.clientId !== req.portalUser!.clientId) { res.status(404).json({ message: 'Not found' }); return; }

  const portalUser = await prisma.clientPortalUser.findUnique({ where: { id: req.portalUser!.clientPortalUserId }, select: { name: true } });
...
router.get('/content', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.put('/content/:id/approve', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const item = await prisma.contentDelivery.findUnique({ where: { id: req.params.id } });
  if (!item || item.clientId !== req.portalUser!.clientId) { res.status(404).json({ message: 'Not found' }); return; }
...
router.put('/content/:id/revision', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { comment } = req.body;
  const item = await prisma.contentDelivery.findUnique({ where: { id: req.params.id } });
  if (!item || item.clientId !== req.portalUser!.clientId) { res.status(404).json({ message: 'Not found' }); return; }
  const updated = await prisma.contentDelivery.update({
    where: { id: req.params.id },
    data: { status: 'NEEDS_REVISION', clientComment: comment?.trim() || null },
  });
  res.json(updated);
});
...
router.get('/invoices', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { status } = req.query;
...
router.get('/notifications', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.put('/notifications/read-all', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  await prisma.clientNotification.updateMany({
    where: { clientPortalUserId: req.portalUser!.clientPortalUserId, read: false },
    data: { read: true },
  });
  res.json({ message: 'All marked as read' });
});

router.put('/notifications/:id/read', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  await prisma.clientNotification.updateMany({
    where: { id: req.params.id, clientPortalUserId: req.portalUser!.clientPortalUserId },
    data: { read: true },
  });
...
router.get('/meeting-types', portalAuthenticate, async (_req, res: Response): Promise<void> => {
  const types = await (prisma as any).meetingType.findMany({ where: { active: true }, orderBy: { createdAt: 'asc' } });
  res.json(types);
});

router.get('/available-slots', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { date, duration } = req.query;
  if (!date || !duration) { res.status(400).json({ message: 'date and duration required' }); return; }
  ...
  for (const avail of admins) {
    const blocked = await (prisma as any).blockedDate.findFirst({
      where: { adminId: avail.adminId, blockedDate: { gte: startOfDay, lte: endOfDay } },
    });
    if (blocked) continue;

    const existingMeetings = await (prisma as any).meeting.findMany({
      where: { adminId: avail.adminId, status: { notIn: ['CANCELLED'] }, startTime: { gte: startOfDay, lte: endOfDay } },
    ...
    const available = allSlots.filter((slot) => {
      ...
      const slotStart = slh * 60 + slm; const slotEnd = slotStart + durationMin;
    ...
    if (available.length > 0) {
      result.push({ adminId: avail.adminId, adminName: avail.admin.name, slots: available });
    }
  }
...
router.get('/meetings', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.post('/meetings', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { adminId, meetingTypeId, title, description, startTime, endTime, timezone, notes } = req.body;
  if (!adminId || !startTime || !endTime) {
    res.status(400).json({ message: 'adminId, startTime, endTime required' }); return;
  }
  const clientId = req.portalUser!.clientId;
  const bookedByPortalUserId = req.portalUser!.clientPortalUserId;

  const toDate = (v: string) => { const d = new Date(v); if (isNaN(d.getTime())) throw new Error('Invalid date'); return d; };
...
router.put('/meetings/:id/cancel', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const { cancelReason } = req.body;
  const meeting = await (prisma as any).meeting.findUnique({ where: { id: req.params.id } });
  if (!meeting || meeting.clientId !== req.portalUser!.clientId) {
    res.status(404).json({ message: 'Meeting not found' }); return;
  }
...
router.get('/crm/orders', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
...
router.get('/crm/stats', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const clientId = req.portalUser!.clientId;
  const orders = await (prisma as any).crmOrder.findMany({
    where: { clientId },
    select: {
      orderAmount: true, netProfit: true, adCost: true, productCost: true, shippingCost: true,
      status: true, paymentStatus: true, source: true, productName: true, quantity: true, createdAt: true,
    },
  });

  const totalOrders = orders.length;
  const totalRevenue = orders.reduce((s: number, o: any) => s + o.orderAmount, 0);
  const totalProfit = orders.reduce((s: number, o: any) => s + o.netProfit, 0);
  ...
  const codPending = orders.filter((o: any) => o.paymentStatus === 'COD_PENDING').reduce((s: number, o: any) => s + o.orderAmount, 0);
  const convRate = totalOrders > 0 ? ((confirmed + delivered + shipped) / totalOrders) * 100 : 0;
  ...
  const recentOrders = orders.filter((o: any) => new Date(o.createdAt) >= sixMonthsAgo);
  const monthly: Record<string, { revenue: number; profit: number; orders: number }> = {};
  for (const o of recentOrders) {
    const key = new Date(o.createdAt).toLocaleString('en-US', { month: 'short', year: '2-digit' });
  ...
  for (const o of orders) sourceMap[o.source] = (sourceMap[o.source] || 0) + 1;

  // Top products
  const productMap: Record<string, { revenue: number; count: number }> = {};
  for (const o of orders) {
    if (!productMap[o.productName]) productMap[o.productName] = { revenue: 0, count: 0 };
  ...
  res.json({
    totalOrders, totalRevenue, totalProfit, totalAdSpend,
    confirmed, delivered, shipped, cancelled, returned, refused,
    codPending,
    ...
    monthlyTrend: Object.entries(monthly).map(([month, v]) => ({ month, ...v })),
...
router.get('/kpis', portalAuthenticate, async (req: PortalRequest, res: Response): Promise<void> => {
  const datePreset = (req.query.datePreset as string) || 'last_7d';
  const config = await prisma.clientKpiConfig.findUnique({ where: { clientId: req.portalUser!.clientId } });

  if (config?.metaToken && config?.metaAdAccountId) {
    try {
      const fields = 'spend,reach,impressions,cpm,cpc,ctr,actions,action_values';
      ...
      const conversionRate = d.reach > 0 ? (leads / d.reach) * 100 : 0;
      ...
      const daily = (dailyData.data || []).map((dd: any) => ({
        ...
        leads: (dd.actions || []).find((a: any) => a.action_type === 'lead')?.value || 0,
        roas: 0,
      }));
      ...
      res.json({
        ...
        datePreset,
        summary: {
          spend, reach: parseInt(d.reach || 0), impressions: parseInt(d.impressions || 0),
          cpm: parseFloat(d.cpm || 0), cpc: parseFloat(d.cpc || 0), ctr: parseFloat(d.ctr || 0),
          leads: parseInt(leads), purchases: parseInt(purchases), roas, costPerLead, conversionRate,
        },
        daily,
      });
      return;
  ...
  // Mock data
  const days = datePreset === 'today' ? 1 : datePreset === 'last_7d' ? 7 : datePreset === 'last_30d' ? 30 : 90;
  const daily = Array.from({ length: days }, (_, i) => {
    ...
    return {
      ...
      leads: Math.floor(8 + Math.random() * 12),
