...
import {
  LayoutDashboard, BarChart2, Image, GitBranch, FileText,
  Bell, LogOut, User, ChevronDown, Menu, X, CalendarClock, ShoppingCart,
} from 'lucide-react';
import stallionLogo from '@/assets/png.png';
import { cn } from '@/lib/utils';
import { usePortalAuth, portalApi } from '@/context/PortalAuthContext';
import { PortalCurrencyProvider, usePortalCurrency } from '@/context/PortalCurrencyContext';
...
function PortalLayoutContent() {
  ...
  useEffect(() => {
    portalApi.get<ClientNotification[]>('/notifications').then(({ data }) => {
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-5 border-b border-slate-800/60">
          <div className="flex items-center gap-2.5">
            <img src={stallionLogo} alt="Stallion Advertising" className="h-9 w-auto object-contain" />
            <div className="text-[10px] text-amber-400 font-medium tracking-widest uppercase">Client Portal</div>
          </div>
        ...
        <div className="px-4 py-4 border-b border-slate-800/60">
          <div className="bg-gradient-to-br from-amber-500/10 to-transparent border border-amber-500/15 rounded-xl p-3">
            <div className="text-xs text-amber-400 font-semibold uppercase tracking-wider mb-0.5">Client</div>
            <div className="text-sm font-bold text-white truncate">{user?.client?.name}</div>
            <div className="text-xs text-slate-500 truncate mt-0.5">{user?.client?.service}</div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3 ml-auto">
                ...
                <div className="absolute right-0 top-11 w-80 bg-[#0d1528] border border-slate-700/50 rounded-xl shadow-2xl z-[60] overflow-hidden">
                  ...
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      <div key={n.id} className={cn('px-4 py-3 border-b border-slate-800/50 last:border-0', !n.read && 'bg-amber-500/5')}>
                        <div className="flex items-start gap-2">
                          {!n.read && <div className="w-1.5 h-1.5 rounded-full bg-amber-400 mt-1.5 flex-shrink-0" />}
                          <div className={cn(!n.read && 'ml-0', n.read && 'ml-3.5')}>
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-sm font-medium text-slate-300 hidden sm:block max-w-24 truncate">{user?.name}</span>
              ...
              {profileOpen && (
                <div className="absolute right-0 top-11 w-48 bg-[#0d1528] border border-slate-700/50 rounded-xl shadow-2xl z-[60] overflow-hidden">
                  <NavLink to="/portal/profile" onClick={() => setProfileOpen(false)}
